...
def hotel_modification_func(app):
    ...
    def hotel_modification():
        ...
        if (isBooked() == False):
            ...
            if data['streetName'] != '':
                updateHotelDetails(uid, data['hotelName'].strip(), data['streetName'].strip(), data['city'].strip(),
                                   data['zipCode'], data['state'].strip(), data['country'].strip())
            else:
                abort(make_response(jsonify(message="Please enter valid street name"), 400))
        else:
            abort(make_response(
                jsonify(message="Cannot update hotel information because there are bookings at the hotel."), 409))
    ...
    def delete_hotel_user():
        ...
        @app.route('/delete_hotel_listing/<listing_id>', methods=['GET', 'POST'])

    def delete_room_listing(listing_id):
        try:
            # Delete listing using listing id
            db.collection('room').document(listing_id).delete()

            uid = getUid()
            hotel_ref = db.collection('user').document(uid)
            hotel_ref.update({"listedRooms": firestore.ArrayRemove([listing_id])})

            return jsonify(message=f"Deleted hotel listing {listing_id}.")
        except Exception as e:
            abort(make_response(jsonify(message=f"Error: {str(e)}"), 400))
